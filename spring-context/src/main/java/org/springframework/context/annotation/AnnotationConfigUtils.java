...
public abstract class AnnotationConfigUtils {
	...
	public static Set<BeanDefinitionHolder> registerAnnotationConfigProcessors(BeanDefinitionRegistry registry, @Nullable Object source) {
		...
		// 获取beanFactory
		// 获取创建完的DefaultListableBeanFactory对象，然后给这个对象的某些属性赋值。
		DefaultListableBeanFactory beanFactory = unwrapDefaultListableBeanFactory(registry);
		if (beanFactory != null) {
			if (!(beanFactory.getDependencyComparator() instanceof AnnotationAwareOrderComparator)) {
				// 设置依赖比较器，AnnotationAwareOrderComparator主要能解析@Order注解和@Priority
				beanFactory.setDependencyComparator(AnnotationAwareOrderComparator.INSTANCE);
			}
			if (!(beanFactory.getAutowireCandidateResolver() instanceof ContextAnnotationAutowireCandidateResolver)) {
				// 设置自动装配解析器，ContextAnnotationAutowireCandidateResolver提供处理延迟加载的功能
				beanFactory.setAutowireCandidateResolver(new ContextAnnotationAutowireCandidateResolver());
			}
		}

		// 创建BeanDefinitionHolder集合，给Spring容器添加Spring内部的特殊Bean对象（7个）
		Set<BeanDefinitionHolder> beanDefs = new LinkedHashSet<>(8);

		// 1.往BeanDefinitionMap注册一个ConfigurationClassPostProcessor(beanFactory 后置处理器，用来完成 bean 的扫描与注入工作)
		...
		// 2.往BeanDefinitionMap注册一个AutowiredAnnotationBeanPostProcessor(bean 后置处理器，用来完成 @AutoWired 自动注入)
		...
		// 3.往BeanDefinitionMap注册一个RequiredAnnotationBeanPostProcessor（已过时）
